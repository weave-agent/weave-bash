...
func (j *BackgroundJob) run(ctx context.Context, cancel context.CancelFunc, command string, args []string, dir string, env []string, direct bool, bus sdk.Bus) {
	defer close(j.done)
	defer cancel()
...
func (bm *BackgroundManager) Start(command string, args []string, dir string, env []string, direct bool, timeout time.Duration, bus sdk.Bus) *BackgroundJob {
	bm.mu.Lock()
	bm.counter++
	...
	go job.run(ctx, cancel, command, args, dir, env, direct, bus)
